...
#define PATH_SIZE 256
  ...
  if (a_test_cnt != 0)
  {
    ...
    do {
      cassert(snprintf(&path[0],PATH_SIZE,"tests/%s/%s",a_test_name,*n_ptr) < PATH_SIZE);

      bool exist;
#if SYSTEM_TYPE == SYSTEM_TYPE_UNIX
      struct stat st;
      exist = stat(&path[0],&st) == 0 && S_ISDIR(st.st_mode);
#elif SYSTEM_TYPE == SYSTEM_TYPE_WINDOWS
      struct _stat st;
      exist = _stat(&path[0],&st) == 0 && st.st_mode & S_IFDIR;
#else
      cassert(false);
#endif
      ...
      {
#if SYSTEM_TYPE == SYSTEM_TYPE_UNIX
        cassert(mkdir(&path[0],0777) == 0);
#elif SYSTEM_TYPE == SYSTEM_TYPE_WINDOWS
        cassert(_mkdir(&path[0]) == 0);
#else
        cassert(false);
#endif
      }
